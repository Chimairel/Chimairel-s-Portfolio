...
export default function BlogPage() {
  ...
  return (
    // Reduced py-8 to py-4 to save top space
    <main className="w-full px-6 py-4 flex justify-center min-h-screen animate-in fade-in duration-500">
      <div className="w-full max-w-6xl flex flex-col gap-6">
        <RetroWindow title="RSS_Feed_Reader.exe" id="blog-main">
          ...
          {/* We will rely on DirectorySearch's internal margins, but let's assume it's compact */}
          <div className="-mb-3"> 
            <DirectorySearch 
          ...
          <div className="flex flex-col gap-4"> {/* Reduced gap from 5 to 4 */}
            {currentItems.map((post) => (
              <div key={post.id} className="flex flex-col md:flex-row border-2 border-border bg-card transition-colors duration-200 hover:shadow-[4px_4px_0px_0px_var(--color-border)] group">
                ...
                {/* Reduced Image Width to 1/4 to give text more room, making the card shorter */}
                ...
                {/* Drastically reduced padding (p-4) and gaps */}
          ...
          {/* Adjusted margin to keep pagination close */}
          <div className="mt-2">
